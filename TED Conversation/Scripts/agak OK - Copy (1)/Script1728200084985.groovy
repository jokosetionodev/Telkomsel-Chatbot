...
if (lastResponse != null) {
	...
	// Validasi apakah teks respons mengandung poin atau konteks yang diinginkan
	String[] validContexts = ["product", "myads", "promo", "layanan"]

	boolean isValidResponse = false
	for (String context : validContexts) {
		if (lastResponseText.toLowerCase().contains(context.toLowerCase())) {
	...
	// Jika respons valid
	if (isValidResponse) {
		println('Respons sesuai dengan konteks yang diharapkan.')
	} else {
		println('Respons tidak sesuai dengan konteks yang diharapkan.')
	}
} else {
